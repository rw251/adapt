...
class HomePage extends React.Component {
  ...
  componentDidMount() {
    ...
    const tabs = initialTabs.map(x => x);

    if(auth.patient.tabs){
      auth.patient.tabs.forEach(tab => {
        if(possibleTabs[tab]) tabs.push(possibleTabs[tab]);
      });
    } else {
      Object.keys(possibleTabs).forEach(tab => {
        tabs.push(possibleTabs[tab]);
      })
    }
  ...
  render() {
    return (
      <div className="HomePage">
        ...
        <Tabs items={this.state.tabs} transform={false} />
